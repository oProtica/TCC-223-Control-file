...
\MyColorBoxBlue{Not all base64 encoders work the same.}{All encoders return a base64 representation of the source characters.  But the length of the returned lines may vary.  A base64 encoded string may span multiple lines.  The only way to ensure a complete base64 line is read, is to read and append lines until an \textbf{ending marker} is read.}
...
\begin{table}[h]
  ...
  \begin{tabular}{l p{0.5\textwidth} l}
    ...
    \MyTempCommand{$<$Exit$>$}{Terminate the current CRM process. If the field is not \textbf{NULL} then output the base64 encoded termination message.}{$</$Exit$>$}\\
    ...
    \MyTempCommand{$<$Save$>$}{Save the current database to where it should be persisted. If the field is NULL by default save to the current database file.}{$</$Save$>$}\\
...
\begin{table}[h]
  ...
  \begin{tabular}{l p{0.5\textwidth} l}
    \MyHline
    \textbf{Starting} & \textbf{Mixed} & \textbf{Ending}\\
    \textbf{Marker} & \textbf{Data} & \textbf{Marker}\\
    \hline
